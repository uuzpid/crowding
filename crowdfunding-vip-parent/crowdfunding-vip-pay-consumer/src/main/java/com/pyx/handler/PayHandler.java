...
import com.pyx.config.PayProperties;
...
public class PayHandler {
    ...
    public String generateOrder(HttpSession session, OrderVO orderVO) throws AlipayApiException {
        // 1.从session域获取以前存储的OrderProjectVO
        OrderProjectVO orderProjectVO = (OrderProjectVO) session.getAttribute("orderProjectVO");

        // 2.将orderProjectVO合并到orderVO
        orderVO.setOrderProjectVO(orderProjectVO);

        // 3.生成订单号并设置到orderVO
        // 根据支付宝规则生成订单号 当前时间+用户id
        // 根据当前时间生成字符串
        String time = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
        String user = UUID.randomUUID().toString().replace("-","").toUpperCase();
        String orderNum = time+user;
        orderVO.setOrderNum(orderNum);
        // 4.计算订单总金额并设置到orderVO
        Double orderAmount = (double)(orderProjectVO.getSupportPrice() * orderProjectVO.getReturnCount() + orderProjectVO.getFreight());
        orderVO.setOrderAmount(orderAmount);

        // 将orderVO存入session域 为了后续保存交易记录到数据库
        session.setAttribute("orderVO",orderVO);
        // 5.调用封装好的方法给支付宝接口发送请求
        String result = sendRequestToAliPay(orderNum, orderAmount, orderProjectVO.getProjectName(), orderProjectVO.getReturnContent());
        return result;
    }
